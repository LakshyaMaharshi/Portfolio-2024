...
const SkillsSection = () => {
  ...
  return (
    <div className="mx-4 md:mx-28 my-16 md:my-48 max-w-6xl space-y-6">
      <h1 className="text-accentColor text-4xl md:text-6xl font-black mb-11">Skills.</h1>
      {/* Tabs */}
      <div className="flex content-center justify-center space-x-4 overflow-x-auto py-10 flex-wrap gap-y-4 ">
        {Object.keys(skillsData).map((category) => (
          <button
            key={category}
            onClick={() => setActiveTab(category)}
            className={`flex-shrink-0 px-4 py-2 text-lg font-medium rounded-lg transition-all ${
              activeTab === category
      ...
      {/* Skill Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 my-12 overflow-visible">
        {skillsData[activeTab].map((skill, index) => (
          ...
          >
            ...
            <span className="text-lg md:text-xl font-medium">{skill.skill}</span>
          </div>
        ))}
      </div>
    </div>
